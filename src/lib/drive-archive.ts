...
export function preloadGoogleSignIn() {
  if (!hasGoogleClientId()) return Promise.resolve();
...
async function driveResponse<T>(res: Response): Promise<T> {
  if (res.status === 401) {
    clearCachedToken();
    writeStored(EMAIL_KEY, "");
    throw new Error("Google sign-in expired. Save again and sign in when prompted.");
  }
  if (!res.ok) {
...
async function listNamedFiles(
  ...
  let pageToken = "";
  do {
    const parts = [`name='${escapeDriveQuery(name)}'`, "trashed=false"];
...
async function trashFile(token: string, id: string) {
  try {
    await driveJson(token, `https://www.googleapis.com/drive/v3/files/${id}`, {
      method: "PATCH",
...
export async function uploadMeetingPdfs(files: { name: string; blob: Blob }[]) {
